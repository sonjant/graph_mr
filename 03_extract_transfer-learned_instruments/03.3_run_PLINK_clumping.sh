# Script to clump GWAS results on Imperial's HPC cluster

#!/bin/bash
#PBS -N j_03_3_run_PLINK_clumping
...
# Clump GWAS results for all 22 chromosomes on folds 1 to n
for i in $(seq 1 $N); do
    exposure=$(sed -n ${i}p < ${input_participants}exposure_list.txt)
    echo "Starting exposure $exposure"

    parallel '
        plink2 \
          --bfile '"${input}"'cleaned_chr{1} \
          --clump '"${output}${exposure}/complete_cases/chr{1}"'.PHENO1.glm.linear \
          --clump-kb 250 \
          --clump-p1 5e-8 \
          --clump-p2 0.01 \
          --clump-r2 0.001 \
          --clump-snp-field ID \
          --out '"${output}${exposure}/complete_cases/chr{1}"'.clumped \
        && rm -f '"${output}${exposure}/complete_cases/chr{1}"'.clumped.nosex 2>/dev/null || true
    ' ::: {1..22}

    echo "Finished exposure $exposure, copying results back..."
    cp -r "${output}${exposure}" "${EPHEMERAL_BASE}/${METHOD_TYPE}_${INPUT_TYPE}/gwas_results/"
done

echo -e "\nFinished running PLINK clumping for exposures 1 to $N \n"
