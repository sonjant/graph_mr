...
[ -z "$GRAPH_MR_VCF_DIR" ] && { echo "Error: GRAPH_MR_VCF_DIR must be set to the directory containing merged.chr*.1kg.vcf.gz (see README)."; exit 1; }
...
snp_list="${RESULTS_BASE}/${METHOD_TYPE}_${INPUT_TYPE}/instrument_dosages/cross_fitted_instruments/all_unique_instruments_list.txt"

# Path to the directory containing the VCF files
vcf_dir="${GRAPH_MR_VCF_DIR}"

# Output directory for selected variants
output_dir="${EPHEMERAL_BASE}/${METHOD_TYPE}_${INPUT_TYPE}/instrument_dosages/cross_fitted_instruments"
mkdir -p "$output_dir"
