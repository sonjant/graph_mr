...
#!/bin/bash
#PBS -N j_02_4_run_PLINK_clumping
...
# Clump GWAS results in parallel for all 22 chromosomes on folds 1 to n
for i in $(seq 1 $N); do
    ...
    for n in {1..10}; do
        ...
        parallel '
            plink2 \
              ...
              --clump '"${output}${exposure}/fold${n}/chr{1}"'.PHENO1.glm.linear \
              --clump-kb 250 \
              --clump-p1 5e-8 \
              --clump-p2 0.01 \
              --clump-r2 0.001 \
              --clump-snp-field ID \
              --out '"${output}${exposure}/fold${n}/chr{1}"'.clumped \
            && rm -f '"${output}${exposure}/fold${n}/chr{1}"'.clumped.nosex 2>/dev/null || true
        ' ::: {1..22}
    done
